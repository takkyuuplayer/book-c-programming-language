...
// Kernighan, Brian W.. C Programming Language (p. 121). Pearson Education. Kindle Edition.
...
void _reverse(void *lineptr[], int size);
...
int main(int argc, char const *argv[])
{
    ...
    if ((nlines = _readlines(lineptr, MAXLINES)) >= 0)
    {
        ...
        if (reverse)
        {
            _reverse((void **)lineptr, nlines - 1);
        }
